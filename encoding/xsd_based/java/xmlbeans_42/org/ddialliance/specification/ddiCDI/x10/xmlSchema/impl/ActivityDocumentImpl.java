/*
 * An XML document type.
 * Localname: Activity
 * Namespace: http://ddialliance.org/Specification/DDI-CDI/1.0/XMLSchema/
 * Java type: org.ddialliance.specification.ddiCDI.x10.xmlSchema.ActivityDocument
 *
 * Automatically generated - do not modify.
 */
package org.ddialliance.specification.ddiCDI.x10.xmlSchema.impl;

import javax.xml.namespace.QName;
import org.apache.xmlbeans.QNameSet;
import org.apache.xmlbeans.XmlObject;

/**
 * A document containing one Activity(@http://ddialliance.org/Specification/DDI-CDI/1.0/XMLSchema/) element.
 *
 * This is a complex type.
 */
public class ActivityDocumentImpl extends org.apache.xmlbeans.impl.values.XmlComplexContentImpl implements org.ddialliance.specification.ddiCDI.x10.xmlSchema.ActivityDocument {
    private static final long serialVersionUID = 1L;

    public ActivityDocumentImpl(org.apache.xmlbeans.SchemaType sType) {
        super(sType);
    }

    private static final QName[] PROPERTY_QNAME = {
        new QName("http://ddialliance.org/Specification/DDI-CDI/1.0/XMLSchema/", "Activity"),
    };


    /**
     * Gets the "Activity" element
     */
    @Override
    public org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity getActivity() {
        synchronized (monitor()) {
            check_orphaned();
            org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity target = null;
            target = (org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity)get_store().find_element_user(PROPERTY_QNAME[0], 0);
            return (target == null) ? null : target;
        }
    }

    /**
     * Sets the "Activity" element
     */
    @Override
    public void setActivity(org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity activity) {
        generatedSetterHelperImpl(activity, PROPERTY_QNAME[0], 0, org.apache.xmlbeans.impl.values.XmlObjectBase.KIND_SETTERHELPER_SINGLETON);
    }

    /**
     * Appends and returns a new empty "Activity" element
     */
    @Override
    public org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity addNewActivity() {
        synchronized (monitor()) {
            check_orphaned();
            org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity target = null;
            target = (org.ddialliance.specification.ddiCDI.x10.xmlSchema.Activity)get_store().add_element_user(PROPERTY_QNAME[0]);
            return target;
        }
    }
}
